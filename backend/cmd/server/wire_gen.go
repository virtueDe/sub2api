...
func initializeApplication(buildInfo handler.BuildInfo) (*Application, error) {
	...
	checkInService := service.NewCheckInService(db, settingRepository, billingCacheService)
	userHandler := handler.ProvideUserHandlerWithCheckIn(userService, authService, emailService, emailCache, affiliateService, serviceUserPlatformQuotaRepository, checkInService)
	...
	imageStorage := service.ProvideImageStorage(imageStorageSettingService)
	imageURLProxyCache := service.ProvideRedisImageURLProxyCache(redisClient)
	openAIGatewayService := service.NewOpenAIGatewayServiceWithImageURLProxy(accountRepository, usageLogRepository, usageBillingRepository, userRepository, userSubscriptionRepository, userGroupRateRepository, gatewayCache, configConfig, schedulerSnapshotService, concurrencyService, billingService, rateLimitService, billingCacheService, httpUpstream, deferredService, openAITokenProvider, grokTokenProvider, modelPricingResolver, channelService, balanceNotifyService, settingService, serviceUserPlatformQuotaRepository, imageStorage, imageURLProxyCache)
	...
	dailyTokenRankingRepository, err := service.ProvideDailyTokenRankingRepository(usageLogRepository)
	if err != nil {
		return nil, err
	}
	...
	settingHandler := handler.ProvideAdminSettingHandlerWithCheckIn(settingService, emailService, turnstileService, aliyunCaptchaService, opsService, paymentConfigService, paymentService, userAttributeService, notificationEmailService, totpService, userService, checkInService)
	...
	asyncImageHandler := handler.ProvideAsyncImageHandler(imageTaskService, openAIGatewayHandler, opsService, gatewayHandler)
