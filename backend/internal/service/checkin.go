...
func (s *CheckInService) GetConfig(ctx context.Context) (CheckInConfig, error) {
	...
	if cfg.RewardMin < 1 || cfg.RewardMin > 1000 {
		cfg.RewardMin = 1
	}
	if cfg.RewardMax < 1 || cfg.RewardMax > 1000 {
		cfg.RewardMax = 1000
	}
	cfg.RewardMin = math.Round(cfg.RewardMin*10) / 10
	cfg.RewardMax = math.Round(cfg.RewardMax*10) / 10
	if cfg.RewardMin > cfg.RewardMax {
		cfg.RewardMin, cfg.RewardMax = 1, 1000
	}
	cfg.Condition = strings.TrimSpace(values[SettingKeyCheckInCondition])
	if cfg.Condition != "request" && cfg.Condition != "consumption" {
		cfg.Condition = ""
	}
...
func (s *CheckInService) UpdateConfig(ctx context.Context, cfg CheckInConfig) (CheckInConfig, error) {
	...
	if cfg.Condition == "request" && (cfg.RequestThreshold < 1 || math.Trunc(cfg.RequestThreshold) != cfg.RequestThreshold) {
		return CheckInConfig{}, infraerrors.BadRequest("CHECK_IN_REQUEST_THRESHOLD_INVALID", "request threshold must be a positive integer")
	}
	if cfg.Condition == "consumption" && cfg.ConsumptionThreshold <= 0 {
		return CheckInConfig{}, infraerrors.BadRequest("CHECK_IN_CONSUMPTION_THRESHOLD_INVALID", "consumption threshold must be greater than 0")
	}
...
func (s *CheckInService) GetStatus(ctx context.Context, userID int64) (CheckInStatus, error) {
	...
	if err := s.db.QueryRowContext(ctx, `SELECT id, username, email, balance FROM users WHERE id = $1 AND deleted_at IS NULL`, userID).Scan(&status.Account.ID, &username, &email, &status.Account.Balance); err != nil {
		return status, err
	}
...
func (s *CheckInService) CheckIn(ctx context.Context, userID int64) (CheckInRecord, CheckInStatus, error) {
	cfg, err := s.GetConfig(ctx)
	if err != nil {
		return CheckInRecord{}, CheckInStatus{}, err
	}
	if !cfg.Enabled {
		return CheckInRecord{}, CheckInStatus{}, ErrCheckInDisabled
	}
	...
	var balance float64
	if err := tx.QueryRowContext(ctx, `SELECT role,status,balance FROM users WHERE id=$1 AND deleted_at IS NULL FOR UPDATE`, userID).Scan(&role, &state, &balance); err != nil {
		return CheckInRecord{}, CheckInStatus{}, err
	}
	if role != RoleUser || state != StatusActive {
		return CheckInRecord{}, CheckInStatus{}, ErrInsufficientPerms
	}
	now := timezone.Now()
	today := now.Format("2006-01-02")
	...
	var exists bool
	if err := tx.QueryRowContext(ctx, `SELECT EXISTS(SELECT 1 FROM check_in_records WHERE user_id=$1 AND check_in_date=$2)`, userID, today).Scan(&exists); err != nil {
		return CheckInRecord{}, CheckInStatus{}, err
	}
	if exists {
		return CheckInRecord{}, CheckInStatus{}, ErrCheckInAlreadyDone
	}
	var requests int64
	var spend float64
	if err := tx.QueryRowContext(ctx, `SELECT COUNT(*), COALESCE(SUM(actual_cost),0) FROM usage_logs WHERE user_id=$1 AND actual_cost > 0 AND created_at >= $2 AND created_at < $3`, userID, dayStart, dayEnd).Scan(&requests, &spend); err != nil {
		return CheckInRecord{}, CheckInStatus{}, err
	}
	...
	reward, err := randomCheckInReward(cfg.RewardMin, cfg.RewardMax)
	if err != nil {
		return CheckInRecord{}, CheckInStatus{}, err
	}
	if _, err = tx.ExecContext(ctx, `UPDATE users SET balance=balance+$1, total_recharged=total_recharged+$1, updated_at=NOW() WHERE id=$2`, reward, userID); err != nil {
		return CheckInRecord{}, CheckInStatus{}, err
	}
...
func (s *CheckInService) ListRecords(ctx context.Context, userID int64, month string, page, pageSize int) ([]CheckInRecord, int64, error) {
	...
	if _, err := time.Parse("2006-01", month); err != nil {
		month = timezone.Now().Format("2006-01")
	}
	where := `WHERE r.check_in_date >= ($1 || '-01')::date AND r.check_in_date < (($1 || '-01')::date + INTERVAL '1 month')`
	args := []any{month}
	idx := 2
	if userID > 0 {
		where += fmt.Sprintf(" AND r.user_id=$%d", idx)
		args = append(args, userID)
		idx++
	}
	var total int64
	if err := s.db.QueryRowContext(ctx, "SELECT COUNT(*) FROM check_in_records r "+where, args...).Scan(&total); err != nil {
		return nil, 0, err
	}
	args = append(args, pageSize, (page-1)*pageSize)
	rows, err := s.db.QueryContext(ctx, fmt.Sprintf(`SELECT r.id,r.user_id,u.email,u.username,r.check_in_date::text,r.reward,r.request_count,r.daily_spend,r.created_at FROM check_in_records r JOIN users u ON u.id=r.user_id %s ORDER BY r.check_in_date DESC,r.id DESC LIMIT $%d OFFSET $%d`, where, idx, idx+1), args...)
	if err != nil {
		return nil, 0, err
	}
	defer func() { _ = rows.Close() }()
	...
	for rows.Next() {
		var r CheckInRecord
		if err := rows.Scan(&r.ID, &r.UserID, &r.Email, &r.Username, &r.CheckInDate, &r.Reward, &r.RequestCount, &r.DailySpend, &r.CreatedAt); err != nil {
			return nil, 0, err
		}
...
func randomCheckInReward(minimum, maximum float64) (float64, error) {
	low, high := int64(minimum*10+0.5), int64(maximum*10+0.5)
	n, err := rand.Int(rand.Reader, big.NewInt(high-low+1))
	if err != nil {
		return 0, err
	}
	return float64(low+n.Int64()) / 10, nil
}
func parseCheckInAmount(raw string, fallback float64) float64 {
